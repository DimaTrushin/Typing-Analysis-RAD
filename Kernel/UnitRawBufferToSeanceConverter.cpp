...
bool CKeysBeingPressed::findIteratorOfEvent(unsigned short VKey,
                                            const_iterator* pIndex) const {
  CKeysBeingPressed::const_iterator& index = *pIndex;
  index = std::find_if( begin(),
                        end(),
                        [&VKey](const CSession::iterator& Iterator){
                          return Iterator->getVKCode() == VKey;
                        });
  if (index == end())
    return false;
  return true;
...
void CRawBufferToSeanceConverter::convert(CRawBuffer* pBuffer,
                                          CSeance* pSeance) {
  if (pBuffer->empty())
    return;
...
void CRawBufferToSeanceConverter::setInternalResources( CRawBuffer* pBuffer,
                                                        CSeance* pSeance) {
...
void CRawBufferToSeanceConverter::releaseAllKeysInCurrentKeyboard() {
  for ( auto VKCode = CurrentKeyboardState_.getBeginVKCode();
        VKCode < CurrentKeyboardState_.getEndVKCode();
        ++VKCode)
    CurrentKeyboardState_.ReleaseKey(VKCode);
}
//---------------------------------------------------------------------------
...
void CRawBufferToSeanceConverter::handleKeyPressing(const CRawKey& RawEvent) {
  CurrentSession().emplace_back(RawEvent);
  PressedKeys_.push_back(--(CurrentSession().end()));
}
//---------------------------------------------------------------------------
