...
#ifndef UnitApproximationH
#define UnitApproximationH
//---------------------------------------------------------------------------
// Include
//---------------------------------------------------------------------------
...
public:
  ...
  CApproximationBase(const CMeanDataContainer& newMeanData);
  void setApproximation(const CMeanDataContainer& newMeanData);
...
CApproximationBase<TModeDefinitions>::
  ...
  std::sort(MeanData.begin(), MeanData.end());
}
//---------------------------------------------------------------------------
...
void
  CApproximationBase<TModeDefinitions>::
    setApproximation(const CMeanDataContainer& newMeanData) {
  MeanData = newMeanData;
  std::sort(MeanData.begin(), MeanData.end());
}
//---------------------------------------------------------------------------
...
double
  ...
  if (!(chunkSize > 0))
    return 0.0;
  return elementsInChunk / (static_cast<double>(MeanData.size()) * chunkSize);
}
//---------------------------------------------------------------------------

template<class TModeDefinitions>
size_t CApproximationBase<TModeDefinitions>::getNumberOfSamples() const {
  return MeanData.size();
}
//---------------------------------------------------------------------------
...
size_t CApproximationBase<TModeDefinitions>::
  ...
  auto itDelimiter =
            std::lower_bound(MeanData.begin(), MeanData.end(), delimiter);
  return itDelimiter - MeanData.begin();
}
//---------------------------------------------------------------------------

template<class TModeDefinitions>
bool CApproximationBase<TModeDefinitions>::isEmpty() const {
  return MeanData.empty();
}
//---------------------------------------------------------------------------
...
double CApproximationBase<TModeDefinitions>::getDiscreteMean() const {
  if (MeanData.empty())
    return 0.0;
  double mean = std::accumulate(MeanData.begin(), MeanData.end(), 0.0);
  mean /= static_cast<double>(MeanData.size());
  return mean;
}
//---------------------------------------------------------------------------
...
double CApproximationBase<TModeDefinitions>::getDiscreteDeviation() const {
  ...
  size_t size = MeanData.size();
  for (size_t index = 0; index < size; ++index)
    deviation += (MeanData[index] - mean) * (MeanData[index] - mean);
...
CApproximationBase<TModeDefinitions>::getChunkOfMeanData(
  ...
  auto leftPosition =
    std::lower_bound(MeanData.begin(), MeanData.end(), ChunkOfValues.first);
  auto rightPosition =
    std::lower_bound(MeanData.begin(), MeanData.end(), ChunkOfValues.second);
...
double
  ...
  if (CBase::MeanData.empty())
    return 0.0;
  double result = 0.0;
  size_t size = CBase::MeanData.size();
  for (size_t index = 0; index < size; ++index)
    result += (Method)(argument, CBase::MeanData[index]);
...
double
  ...
  size_t size = CBase::MeanData.size();
  for (size_t index = 0; index < size; ++index)
    result += evaluateAt<derivative>(argument, CBase::MeanData[index]);
  result /= static_cast<double>(size);
  return result;
}
//---------------------------------------------------------------------------
...
double
  ...
  size_t size = CBase::MeanData.size();
  for (size_t index = 0; index < size; ++index)
    result += evaluateUniformAt<derivative>(argument, CBase::MeanData[index]);
  result /= static_cast<double>(size);
  return result;
}
//---------------------------------------------------------------------------
